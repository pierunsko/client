...
public class JumpCircles {
    ...
    // Texture locations matching SoupAPI assets
    private static final ResourceLocation TEX_CIRCLE       = new ResourceLocation("scout", "textures/jump_circles/circle.png");
    private static final ResourceLocation TEX_CIRCLE_BOLD  = new ResourceLocation("scout", "textures/jump_circles/circle_bold.png");
    private static final ResourceLocation TEX_HEXAGON      = new ResourceLocation("scout", "textures/jump_circles/hexagon.png");
    private static final ResourceLocation TEX_PORTAL       = new ResourceLocation("scout", "textures/jump_circles/portal.png");
    private static final ResourceLocation TEX_SCIFI        = new ResourceLocation("scout", "textures/jump_circles/scifi.png");
    ...
    private ResourceLocation getTexture() {
        switch (Scout.config.jumpCirclesStyle) {
            case CIRCLE_BOLD: return TEX_CIRCLE_BOLD;
            case HEXAGON:     return TEX_HEXAGON;
            case PORTAL:      return TEX_PORTAL;
            case SCIFI:       return TEX_SCIFI;
            default:          return TEX_CIRCLE;
        }
    }
    ...
    private static class JumpCircle {
        ...
        JumpCircle(Vec3 pos) {
            ...
            this.angularVelocity = (float)Math.toRadians(Scout.config.jumpCirclesSpinSpeed);
            this.fadeOut       = Scout.config.jumpCirclesFadeOut;
        }

        boolean isExpired() {
            return (System.currentTimeMillis() - startTime) / 1000f > Scout.config.jumpCirclesLiveTime;
        }
        ...
        void render(float pt, double cx, double cy, double cz) {
            ...
            float liveTime      = Scout.config.jumpCirclesLiveTime;
            ...
            if (remaining > 0.3f) {
                rotationAngle -= angularVelocity * normalDelta;
            } else {
                rotationAngle += angularVelocity * normalDelta * (remaining / 0.3f);
            }

            float ageFrac = MathHelper.clamp_float((elapsed / (liveTime / 3f)) * 3f, 0f, 1f);
            float scaleMult = Scout.config.jumpCirclesScale / 100f;
            float radius = ageFrac * scaleMult;
            if (remaining < 0.3f) radius *= remaining / 0.3f;

            float colorAnim = fadeOut ? 1f - remaining : 1f;
            int alpha = (int)(Scout.config.jumpCirclesAlpha * 2.55f * colorAnim);  // 0-100 → 0-255

            double x = position.xCoord - cx;
            double y = position.yCoord - cy;
            double z = position.zCoord - cz;
            ...
            Color c1 = Palette.getColor(0f),    c2 = Palette.getColor(0.33f);
            Color c3 = Palette.getColor(0.66f), c4 = Palette.getColor(1f);
